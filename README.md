# Проект "Мои банковские операции"

## Описание:

Проект "Мои банковские операции" - это виджет на Python, 
который показывает несколько последних успешных банковских операций клиента.


## Установка:

+ Установите на свой компьютер GitHub.
+ Клонируйте репозиторий с GitHub  с помощью веб-URL.
```
 
```


## Использование:

В текущем виде, программа запускается в модуле main.py:



## Примеры использования функций:

+ Модуль masks
содержит функции для наложения масок на конфиденциальные данные:

**get_mask_card_number(card_number: int) -> str**

Принимает на вход номер карты в виде числа и возвращает маску номера по правилу XXXX XX** **** XXXX 
 

**get_mask_account(card_number: int) -> str**

Принимает на вход номер счета и возвращает маску номера по правилу **XXXX


+ Модуль widget
содержит функции для работы с банковскими данными:

**mask_account_card(info: str) -> str**

Принимает  строку, содержащую тип и номер карты или счета, и возвращает строку с замаскированным номером.


**get_date(data: str) -> str**

Принимает на вход строку с датой в формате "2024-03-11T02:26:18.671407"
и возвращает строку с датой в формате "ДД.ММ.ГГГГ" ("11.03.2024").


+ Модуль processing
содержит  функции для фильтрации и сортировки банковских данных:

**filter_by_state(list dict: list[dict[str, Any]], state: str = 'EXECUTED') -> list[dict[str, Any]]**

Фильтрует список операций по статусу выполнения (например по 'EXECUTED')


**sort_by_date(list_dict: list[dict], descending: bool = True) -> list[dict]:**

Сортирует список операций по дате (например по убыванию)


+ Модуль generators
содержит функции, реализующие генераторы для обработки данных.:

**filter_by_currency(transactions: List[Dict[str, Any]], currency: str) -> Iterator[Dict[str, Any]]**

Фильтрует транзакции по заданной валюте

**transaction_descriptions(transactions: List[Dict]) -> Generator[str, None, None]**

Фильтрует транзакции по описанию операции

**card_number_generator(start: int, end: int) -> Generator[str]**

Гененерирует номера карт в заданном диапозоне


+ Модуль decorators
Модуль будет использоваться для размещения декораторов

**log(filename: Optional[str] = None) -> Callable**

Декоратор автоматически логирует начало и конец выполнения функции,
а также ее результаты или возникшие ошибки
      
+ Модуль transaction_reader
 Модуль реализует считывание финансовых операций из CSV- и XLSX-файлов.

**reader_csv_file(path_csv_file: str) -> List[Dict[str, Any]]**
Функция для считывания финансовых операций из CSV.

**reader_excel_file(path_excel_file: str) -> List[Dict[str, Any]]**
Функция для считывания финансовых операций из Excel.

+ Модуль main
**main() -> None**
Функция, которая отвечает за основную логику проекта с пользователем и связывает функциональности между собой.

## Разработка:

+ Проект находится в стадии разработки. 


## Тестирование:
  

Тесты созданы в пакете tests с использованием библиотеки pytest.
В тестах используются фикстуры и параметризация. 
  
**Запуск тестов:**
```bash
pytest

```

**Запустить тесты с выводом покрытия в терминал**
```bash
pytest --cov=src --cov-report=term-missing

```


